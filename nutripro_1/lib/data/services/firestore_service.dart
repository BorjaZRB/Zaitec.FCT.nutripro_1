...
class FirestoreService {
  ...
  Future<void> createDashboardFromSurvey(String uid) async {
    ...
    final int calorieGoal = (data['calorieGoal'] is num) ? (data['calorieGoal'] as num).toInt() : 2000;
    ...
    final progress = _initialGoalProgress(calorieGoal, waterGoal);
    final weekly = _buildWeekly(calorieGoal);
    final monthly = _buildMonthly(calorieGoal);
  ...
  double _initialGoalProgress(int calorieGoal, int waterGoal) {
    double score = (calorieGoal / 3000) * 0.6 + (waterGoal / 4) * 0.4;
    if (score < 0.05) score = 0.05;
    if (score > 0.95) score = 0.95;
    return score;
  }

  List<double> _buildWeekly(int calorieGoal) {
    return List.generate(7, (i) {
      double val = (calorieGoal / 100.0) * 0.01 + (i % 3) * 1.0;
      return double.parse(val.toStringAsFixed(1));
    });
  }

  List<double> _buildMonthly(int calorieGoal) {
    return List.generate(6, (i) {
      double val = (calorieGoal / 100.0) * 0.02 + i * 2.0;
      return double.parse(val.toStringAsFixed(1));
    });
  }
